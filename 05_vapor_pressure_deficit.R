...
# ---------------------------------------------------------------------------
# 3. Build annual mean VPD images (1980 - 2024)
# ---------------------------------------------------------------------------
years <- ee$List$sequence(1980, 2024)
...
# ---------------------------------------------------------------------------
# Use reduceRegions over the Kericho feature collection. Using mean() here but
# users may prefer min(), median(), or a zonal histogram depending on needs.
...
# ---------------------------------------------------------------------------
# Earth Engine often uses band names like "vpd_mean_1980" — ensure extraction
# extracts the final numeric year from the column name.

# Keep only columns that contain 'vpd' (in case metadata columns exist)
...
df <- stats_df %>%
  ...
  pivot_longer(cols = everything(),
               ...
               values_to = "vpd") %>%
  mutate(
    # Extract year number from band name (e.g. 'vpd_mean_1980' -> 1980)
